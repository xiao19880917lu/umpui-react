        ...
        switch (this.props.modalCon.type) {
            ...
            case 'html':
                return <div className="tip"
                    dangerouslySetInnerHTML={Utils.createMarkup(this.props.modalCon.html)}></div>;
            case 'form':
                var liList = [];
            ...
            case 'checkbox':
                ...
                $.each(this.props.item, function (key, value) {
                    ...
                    liList.push(<li><ReactCheckbox ref={key} key={key} name={key}
                        checked={checked} label={label}></ReactCheckbox></li>);
                });
    ...
    generateBtn: function () {
        ...
        switch (config.type) {
            case 'warning':
                ...
                break;
            case 'custom':
                for (var key of Object.keys(config.btns)) {
                    btnList.push(<button data-name={key} className="mb-sm btn btn-primary">
                            {config.btns[key]}</button>);
                }
                break;
            default:
