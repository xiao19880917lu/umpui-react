    ...
    generatorRow() {
        ...
        for (let key in this.props.showTags) {
            ...
            if (key === 'operation' && (value.display || value.display == null)) {
                ...
                let spanSort = [];
                const sortAscCss = classNames({
                    'fa fa-sort-asc': true,
                    'umpui-asc': this.state.sort && (key === this.state.sortField)
                });
                const sortDescCss = classNames({
                    'fa fa-sort-desc': true,
                    'umpui-desc': !this.state.sort && (key === this.state.sortField)
                });
                if (value.sort) {
                    spanSort.push(<span className={sortAscCss} key="sortasc"></span>);
                    spanSort.push(<span className={sortDescCss} key="sortdesc"></span>);
                    thList.push(<th key={key}><UDnD field={key}>{value['title']}
                        <div className="umpui-sortcon" onClick={this.sortClick.bind(this, key)}>
                            {spanSort}</div></UDnD></th>);
                } else {
                    thList.push(<th key={key}><UDnD field={key}>{value['title']}</UDnD></th>);
                }
            } else if (typeof(value) === 'string') {
                thList.push(<th key={key}><UDnD field={key}>{value}</UDnD></th>);
            }
        }
        let operationArr = [];
        if (this.props.tableCfg.cfg && this.props.tableCfg.cfg.checkBox) {
            operationArr.push(<span key="thcheckbox"><input type="checkbox"
                    checked={this.props.checked} onClick={this.checkAll.bind(this)}/></span>);
        }
