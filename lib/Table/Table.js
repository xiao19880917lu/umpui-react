...
export default class Table extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            exportConfig: this.getExportConfig(arrData.size),
            flag: 0,
            filter: false,
            checkAll: false,
            spinning: false,
        ...
        if (Immutable.List.isList(this.state.currPageData)) {
            ...
            let rows = content.map(function (row, index) {
                ...
                TrRows.push(<TrRow obj={row} index={index} key={row.id} id={row['id']} checked={checked}
                    callback={self.checkRow.bind(self)} tableCfg={self.state.tableCfg} expandAll={self.state.expandAll}
    ...
    tableHeadGenerator() {
        ...
        if (display && display.switchTags) {
            divList.push(<div key="switchTags" className="header-extra" onClick={this.switchTags.bind(this)}>
                <Tooltip  title="设置显示字段" placement={'top'}>
                    <i className="fa fa-cog" title="设置显示字段"></i></Tooltip></div>);
        }
        if (display && display.export) {
            divList.push(<div key="export" className="header-extra">
                <Tooltip  title="下载数据" placement={'top'}>
        ...
        if (display && display.refresh) {
            divList.push(<div key="refresh" className="header-extra" onClick={this.refreshTable.bind(this)}>
                <Tooltip  title="刷新"  placement={'top'}>
                    <i className="fa fa-refresh" title="刷新"></i></Tooltip></div>);
        }
        if (display && display.setPageSize) {
            divList.push(<div key="setPageSize" className="header-extra" onClick={this.showSetPageSize.bind(this)}>
                <Tooltip  title="页面设置"  placement={'top'}>
                    <i className="fa fa-cogs" title="页面设置"></i></Tooltip></div>);
        }
