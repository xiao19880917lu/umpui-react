...
export default class Table extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (this.props.tableCfg.url) {
            ...
            !bProps && this.getData(null, null, nextProps);
        }
    }
            ...
            $.ajax({
                ...
                success(res) {
                    if (res.status * 1 === 0) {
                        ...
                        // let exportConfig = Object.assign(self.state.exportConfig,);
                        let temp = {
    ...
    tableHeadGenerator() {
        ...
        if (display && display.refresh) {
            divList.push(<div key="refresh" className="header-extra" onClick={this.refreshTable.bind(this)}>
                    <i className="fa fa-refresh" title="刷新"></i></div>);
        }
        if (display && display.setPageSize) {
            divList.push(<div key="setPageSize" className="header-extra" onClick={this.showSetPageSize.bind(this)}>
                    <i className="fa fa-cogs" title="页面设置"></i></div>);
        }
