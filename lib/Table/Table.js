...
export default class Table extends React.Component {
    ...
    getExportConfig() {
        ...
        if (!tableCfg.url || tableCfg.cfg.pageType !== 'server') {
            return {
                headers: objHeaders,
                data: this.state && this.state.content ? this.state.content.toJS() : (this.props.content || []),
    ...
    changeData(currentPage) {
        ...
        if (this.props.tableCfg.cfg.pageType === 'server') {
            ...
            this.setState({
                currPageData: Immutable.fromJS(curData),
                checkAll: isCheckAll
            });
        }
    }
    ...
    isCheckAll(curData, rowState) {
        // update at 2016/09/26 by @liuzechun <liuzechun@baidu.com>
        /*for (let i = 0, len = curData.size; i < len; i++) {
            if (!rowState.hasOwnProperty(curData['id'])) {
                return false;
            }
        }*/
        // curData好像是一个Immutable对象
        // Object.prototype.toString.call(curData).indexOf('Array') !== -1
        // curData instanceof Array
        let pageData = (curData instanceof Array) ? curData : curData.toJS();
        for (let i = 0, len = pageData.length; i < len; i++) {
            if (!rowState.hasOwnProperty(pageData[i]['id']) || rowState[pageData[i]['id']] === false) {
                return false;
            }
        }
        return true;
    ...
    tableHeadGenerator() {
        ...
        if (title) {
            ...
            result.push(typeof display.retract !== 'undefined'
                ...
                : <div key="table-title" className="table-header">
                    <span>{title}</span>
                </div>
            );
        }
