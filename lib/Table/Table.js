...
export default class Table extends React.Component {
    ...
    tableHeadGenerator() {
         ...
         */
        let arrWidgets = display && display.basic;
        // 为了美观，如果有自定义的控件，把控件放到过滤框之后，其他控件之前
        if (arrWidgets.indexOf('filter') !== -1) {
            let props = {
                name: 'filter',
                placeholder: '要过滤的内容',
        ...
        if (this.props.children){
            for (let v of this.props.children){
        ...
        if (arrWidgets.length > 0) {
            if (arrWidgets.indexOf('editTable') !== -1) {
                let arrList = [];
            ...
            if (arrWidgets.indexOf('refresh') !== -1) {
            ...
            if (arrWidgets.indexOf('fullScreen') !== -1) {
            ...
            if (arrWidgets.indexOf('export') !== -1) {
            ...
            if (arrWidgets.indexOf('switchTags') !== -1) {
        ...
        if (gearsList.length > 0) {
            ...
            divList.push(<div className="umpui-header-extra" key="umpui-table-menu"
                    ...
                    <Tooltip title="控件菜单" placement="top">
                        <span><i className="fa fa-list"></i>菜单</span>
                    </Tooltip>
                    <ul className={listClass}>{gearsList}</ul></div>);
        }
