...
export default class Table extends React.Component {
            ...
            $.ajax({
                ...
                success(res) {
                    if (res.status * 1 === 0) {
                        ...
                        let tempExConfig = {};
                        let cfg = self.state.tableCfg.cfg;
    ...
    tableHeadGenerator() {
        ...
        if (display && display.refresh) {
            menuList.push(<li key="refresh" onClick={this.refreshTable.bind(this)}>
                <i className="fa fa-refresh" title="刷新"></i>
                <span className="umpui-span-left">刷新表格</span></li>);
        }
        if (display && display.setPageSize) {
            menuList.push(<li key="setPageSize" onClick={this.showSetPageSize.bind(this)}>
        ...
        if (display && display.editTable) {
            ...
            if (this.state.editTable) {
                arrList.push(<div className="umpui-edit" onClick={this.cancelEdit.bind(this, null)} key={'cancelEdit'}>
