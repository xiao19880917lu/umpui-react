...
var Table = React.createClass({
    ...
    requestData: function () {
        let self = this;
        let curCount = 0;
        let dataParams = $.extend({}, self.props.tableCfg.params, {
                pageNum: -1,
                page: -1,
                pageSize: self.cfg.size ? self.cfg.size : 0
            });
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.status * 1 === 0 && res.data.length > 0) {
                        ...
                        let val = parseFloat(curCount / self.state.count * 100).toFixed(2);
    ...
    tableHeadGenerator: function () {
        ...
        if (display && display.export) {
            divList.push(<div className="header-extra" onClick={this.confirmExport}>
                    <i className="fa fa-file-excel-o"></i></div>);
        }
        return divList;
    },
    render: function () {
        ...
        return (
            <div className="panel panel-default reactTable">
                ...
                {this.state.switchTags && <ReactModal modalCon={modalCon}
                    item={this.showTags} handleModalClick={this.setShowTags}/>}
                {this.state.confirmExport && <ReactModal ref={'confirmExport'} modalCon={progressModalProps}
                    handleModalClick={this.exportData}/>}
                <div style={progressStyle}>{display && display.export && <ReactProgress ref={'progressbar'}/>}</div>
