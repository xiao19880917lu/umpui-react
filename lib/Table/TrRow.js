...
var TrRow = React.createClass({
    ...
    generatorRow: function () {
            ...
            if (typeof v === 'object' && v.display === false) {
                ...
                switch (v.type) {
                    ...
                    case 'JSON':
                        ...
                        tdList.push(<td key={k} data-key={k}><pre className="json"
        ...
        if (self.props.tableCfg.cfg && self.props.tableCfg.cfg.checkBox) {
            ...
            operationSpan.push(<span><input type="checkbox" checked={self.props.checked}
                    onChange={this.checkIt} disabled={disabled}/></span>);
        }
        if (self.props.tableCfg.display.tips) {
            var tips = this.props.obj['tips'];
            operationSpan.push(<span data-key={data['id']} className='fa fa-question-circle'
                    onMouseEnter={this.handleMouseEvent.bind(this, tips, 'enter')}
                    onMouseLeave={this.handleMouseEvent.bind(this, tips, 'leave')}></span>);
        }
        if (self.props.tableCfg.display && self.props.tableCfg.display.expand) {
            var strClaName = 'fa fa-caret-right';
            operationSpan.push(<span data-key={data['id']} className={strClaName}
    ...
    handleMouseEvent: function (tips, type, e) {
        var elem = $(ReactDOM.findDOMNode(this.refs.tr));
        switch (type) {
            case 'enter':
                var data = {
                    server1: '已执行完',
    ...
    render: function () {
        ...
        var disabled = this.props.obj['disabled'];
        var tips = this.props.obj['tips'];
        if (disabled) {
