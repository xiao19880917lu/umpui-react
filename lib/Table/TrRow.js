...
export default class TrRow extends React.Component {
    ...
    /**
     * radio没有直接获取事件之后的值，需要通过e获取，封装一层
     * @param {string} field  要编辑的字段
     * @param {Object} e Event对象
     */
    setRadioGroupData(field, e) {
        e = e || window.event;
        e.stopPropagation();
        e.preventDefault();
                        ...
                        switch (v.editCfg['elemType']) {
                            ...
                            case 'radioGroup':
                                ...
                                arrEdit.push(<RadioGroup value={tdData}
                                        onChange={this.setRadioGroupData.bind(this, k)}>{arrRadioList}</RadioGroup>);
                            case 'select':
                                break;
                        }
                    }
    ...
    render() {
        ...
        // let funcChange = isShowCheckbox && !disabled ? function() {} : function () {};
        let funcChange = function () {};
        let style = disabled ? {background: '#e4e5e7'} : {};
        return (<tr ref="tr" style={style} onClick={funcChange.bind(this)}>{tdList}</tr>);
    }
}
