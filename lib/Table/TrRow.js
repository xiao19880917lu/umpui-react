...
export default class TrRow extends React.Component {
    ...
    cusOperaEvent(data, action, event) {
        ...
        action.onClick && action.onClick(null, data);
    }
    checkIt() {
                ...
                if (k === 'operation') {
                    ...
                    actions.forEach(function (action, dex) {
                        actionList.push(<a href="#" className=""
                            ...
                            {action['title']}</a>);
                        /*let isActive = (dex === self.state.activeAction) ? true : false;
                        actionList.push(<span onClick={self.handleComponent.bind(self, dex)}>{
                            action['title']}</span>)
                        if (isActive) {
                            UserMixin.wrapContainer(action.component, data);
                        }*/
                    });
