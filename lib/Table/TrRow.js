    ...
    generatorRow: function () {
        ...
        $.each(self.props.showTags, function (k, v) {
            ...
            if (typeof v === 'object' && v.display === false) {
               ...
               if (v.render) {
                   tdList.push(<td key={v} data-key={k}>{v.render(tdData, data)}</td>);
               }else {
                   tdList.push(<td key={v} data-key={k}>{tdData}</td>);
               } 
            } else {
                if (k === 'operation') {
                    var Links = [];
        ...
        if (self.props.tableCfg.display && self.props.tableCfg.display.tips) {
            ...
            operationSpan.push(<span key="trtips" data-key={data['id']} className='fa fa-question-circle'
