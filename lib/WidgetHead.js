            ...
            if (operationData.hasOwnProperty(key)) {
                if (key === 'refresh') {
                    operationList.unshift(<li key={key} className="widget-operation">
                            <span className='cursor-pointer'><i className={operationData[key]}
                            onClick={this.onRefreshed.bind(this)}></i></span></li>);
                }
                if (key === 'expand') {
                    operationList.unshift(<li key={key} className="widget-operation">
                            <span className='cursor-pointer'><i className={this.state.view}
                            ref="expandOrCompress" onClick={this.onExpanded.bind(this)}></i></span></li>);
                }
                if (key === 'edit') {
                    operationList.unshift(<li key={key} className="widget-operation">
                            <span className='cursor-pointer'><i className={operationData[key]}
                            onClick={this.onEdited.bind(this)}></i></span></li>);
                }
                if (key === 'delete') {
                    operationList.unshift(<li key={key} className="widget-operation">
                            <span className='cursor-pointer'><i className={operationData[key]}
                             onClick={this.onDeleted.bind(this)}></i></span></li>);
                }
            }
        }
