...
class WidgetBody extends React.Component {
    ...
    generateBody() {
        let that = this;
        let bodyType = this.props.bodyConf.bodyType;
        let bodyConfig = this.props.bodyConf.bodyContent;
        let bodyId = this.props.bodyConf.bodyId;
        switch (bodyType) {
            case 'text':
                return <div>{bodyConfig}</div>;
            case 'chart':
                return (<div>{that.generateOperation()}
                        <ReactHighcharts config={bodyConfig}
                        ref={'widgetBody_chart_' + bodyId}/></div>);
            case 'stock':
                return (<div>{that.generateOperation()}
                        <ReactHighstock config={bodyConfig}
                        ref={'widgetBody_stock_' + bodyId}/></div>);
            case 'business':
                return (<Business defaultUrl={bodyConfig.defaultUrl}
                    searchUrl={bodyConfig.searchUrl} />);
            default:
                return null;
        }
    }
