...
export default class FormSlider extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        this.setState({sliderConfig: nextProps.formConfig, step: 0, storeData: []});
    }
    getVal() {
    ...
    handleNextClick(nextFormId) {
        ...
        if (errMsg.length === 0) {
            ...
            if (sliderStepsConfig[this.state.step].isFinal) {
                this.props.submit && this.props.submit(this.state.storeData, sliderStepsConfig[this.state.step]);
                this.setState({errMsg: '', step: 0, storeData: []});
            } else {
                if (nextFormId) {
