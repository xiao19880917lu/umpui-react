...
export default class ReactForm extends React.Component {
                ...
                case 'list':
                    ...
                    for (let i in item.list) {
                        if (item.list.hasOwnProperty(i)) {
                            if(item.trashIcon) {
                                articleList.push(<article><div>{item.list[i]}</div><div><label><i className="fa fa-trash-o" onClick={self.handleTrash.bind(self,item.list[i])}></i></label></div></article>);
                            }else {
                                articleList.push(<article><div>item.list[i]</div></article>);
                            }
                        }
                    }
                    formList.push(<div key={item.ref + dex}><div className="opmDashboardAddedWidgetsList" id="dashboards_list">{articleList}</div></div>);
            }
        });
        return formList;
    }
    ...
    render() {
        let strHeader = <span>{this.props.config && this.props.config.title ? this.props.config.title : ''}{this.props.config && this.props.config.removeIcon ? <i onClick={this.handleRemove.bind(this)}className='fa fa-times right-remove'></i> : ''}</span>;
        return (
