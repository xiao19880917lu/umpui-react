...
export default class Business extends React.Component {
    ...
    componentDidMount() {
        ...
        let d3Data = this.handlerData(data.data);
        ...
        let links = [
            {'source': {'x': 50, 'y': 80}, 'target': {'x': 30, 'y': 211}, 'key': 1, 'size': 1}, // Alice → Bob
            {'source': {'x': 30, 'y': 211}, 'target': {'x': 200, 'y': 110}, 'key': 2, 'size': 1} // Bob → Carol
        ];
        this.setState({nodes: d3Data.nodes, links: d3Data.links});
    }
    handlerData(baseGraphData) {
        let myThis = this;
        // 将已有格式数据转化为d3 格式数据
        // 计算zone节点位置
        let cellWidth = myThis.state.width / (_.keys(baseGraphData.nodes).length * 2 + 1);
        ...
        let radius = 100;
        let center = {x: myThis.state.width / 2, y: myThis.height / 2};
        // 确定 zone 圆型布局的位置
        let zonePositionMap = {};
    ...
    onSearch(value) {
        ...
        let resultData = this.handlerResultData(initialData, data[0].data);
    ...
    render() {
        return (
              <div>
                <BusinessForm onSearch={this.onSearch.bind(this)}/>
                <Graph nodes={this.state.nodes} links={this.state.links} width={this.props.width}/>
              </div>
        );
    }
}